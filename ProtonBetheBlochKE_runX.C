#define ProtonBetheBlochKE_run5387_cxx
#include "ProtonBetheBlochKE_run5387.h"
...
void ProtonBetheBlochKE_run5387::Loop() {
	if (fChain == 0) return;
	...
	TH1D *h1d_kebb_stop=new TH1D("h1d_kebb_stop","", nke, kemin, kemax); //calorimetric-based calc. (stopping protons)
	TH1D *h1d_kebb_recoinel=new TH1D("h1d_kebb_recoinel","", nke, kemin, kemax); //calorimetric-based calc. (reco. inel. protons)
	TH1D *h1d_kebb_recoel=new TH1D("h1d_kebb_recoel","", nke, kemin, kemax); //calorimetric-based calc. (reco. inel. protons)
	...
	const int n_eloss=40;
        ...
        double dEloss=0.5;
	vector<double> Eloss;
	TH1D *h1d_KEend_tune[n_eloss];
	for (int k=0; k<n_eloss; ++k) {
                double eloss_step=Eloss_start+(double)k*dEloss;
                Eloss.push_back(eloss_step);

		h1d_KEend_tune[k]=new TH1D(Form("h1d_KEend_tune_%d",k),"", ndke, dkemin, dkemax);
	}
	...
	double mean_Elosscalo_stop=(4.43214e+01)/(9.61985e-01); //from fit result[with beam xy cut]
	...
	for (Long64_t jentry=0; jentry<nentries;jentry++) { //evt loop
		...
		double KEff=ke_beam_MeV-mean_kerange_stop;
		double KEcalo_reco_constEcalo=-1; KEcalo_reco_constEcalo=BB.KEAtLength(KEff, range_reco);
		...
		if (IsBeamXY&&IsBQ&&IsCaloSize&&IsPandoraSlice) {
			...
			for (int k=0; k<n_eloss; ++k) {
				h1d_KEend_tune[k]->Fill(ke_beam_MeV-Eloss.at(k)-ke_calo_MeV);
			}
			...
			if (IsRecoStop) {
				...
				h1d_kebb_stop->Fill(ke_calo_MeV);
			...
			if (IsRecoInEL) { //reco_inel
				h1d_kebb_recoinel->Fill(ke_calo_MeV);
			...
			if (IsRecoEL) { //reco_el
				h1d_kebb_recoel->Fill(ke_calo_MeV);
	...
	//TFile *fout = new TFile(Form("data_kebb_temp.root"),"RECREATE"); //temp allowed dE<0
	//TFile *fout = new TFile(Form("data_kebb.root"),"RECREATE"); //set ke=-700 if dE<0, ke<0 if keff_reco is under-estimated but longer reco trklen 
	TFile *fout = new TFile(Form("data_kebb_beamxy.root"),"RECREATE"); //set ke=-700 if dE<0, ke<0 if keff_reco is under-estimated but longer reco trklen 
		h1d_kebeam->Write();
