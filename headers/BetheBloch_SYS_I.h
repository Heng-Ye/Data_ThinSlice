...
class BetheBloch_SYS_I {

 public:

  BetheBloch_SYS_I();
  BetheBloch_SYS_I(int pdg);
...
BetheBloch_SYS_I::BetheBloch_SYS_I()
  : pdgcode(0)
  , mass(0)
  , charge(0)
  , sp_KE_range(0)
...
BetheBloch_SYS_I::BetheBloch_SYS_I(int pdg)
  : pdgcode(0)
  , mass(0)
  , charge(0)
  , sp_KE_range(0)
...
void BetheBloch_SYS_I::SetPdgCode(int pdg){
...
double BetheBloch_SYS_I::densityEffect(double beta, double gamma){
...
double BetheBloch_SYS_I::betaGamma(double KE){
...
double BetheBloch_SYS_I::meandEdx(double KE){
...
double BetheBloch_SYS_I::MPVdEdx(double KE, double pitch){
...
double BetheBloch_SYS_I::IntegratedEdx(double KE0, double KE1, int n){
...
double BetheBloch_SYS_I::RangeFromKE(double KE){

  return IntegratedEdx(0, KE);
}

void BetheBloch_SYS_I::CreateSplines(int np, double minke, double maxke){
...
double BetheBloch_SYS_I::RangeFromKESpline(double KE){
  if (!sp_KE_range){
...
double BetheBloch_SYS_I::KEFromRangeSpline(double range){
  if (!sp_range_KE){
...
double BetheBloch_SYS_I::KEAtLength(double KE0, double tracklength){
...
void BetheBloch_SYS_I::CreateSplineAtKE(int iKE){
...
double BetheBloch_SYS_I::Fit_dEdx_Residual_Length(const vector<double> dEdx, const vector<double> ResRange, int PID, bool save_graph) {
...
double BetheBloch_SYS_I::Landau_xi(double KE, double pitch){
  double K = 0.307;
  double rho = 1.396;
  double Z = 18;
  double A = 39.948;
...
double BetheBloch_SYS_I::Get_Wmax(double KE){
  double me = 0.511; //MeV me*c^2
  double gamma = (KE/mass)+1.0;
  double beta = TMath::Sqrt(1-(1.0/(gamma*gamma)));
...
double dEdx_PDF_fuction2(double *x, double *par){
...
double BetheBloch_SYS_I::dEdx_PDF(double KE, double pitch, double dEdx){
  ...
  TF1 *PDF = new TF1("", dEdx_PDF_fuction2, -100., 1000., 5);
...
double BetheBloch_SYS_I::Fit_Proton_Residual_Length_Likelihood(const vector<double> dEdx, const vector<double> ResRange, int PID, bool save_graph){
  // == only for protons
  int abs_PID = abs(PID);
  //if(!abs(PID) == 2212){
  if(PID != 2212){
