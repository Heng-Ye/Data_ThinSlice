#define ProtonAfterMomentumReweight_run5387_cxx
#include "ProtonAfterMomentumReweight_run5387.h"
...
void ProtonAfterMomentumReweight_run5387::Loop() {
	if (fChain == 0) return;
	...
	double sg_denom_data=47.075678784947826; //fit at ff

	double mu_nom_data=390.81237292943916; //KEbeam-const E data
	double sg_nom_data=47.52091718691363; //KEbeam-const E data

	double mu_nom_mc=388.560260293186; //for mc
	double sg_nom_mc=43.13168235197187; //formc

	//weighting func. (ke)
	TF1 *kerw=new TF1(Form("kerw"),govg,0,800,4);
	kerw->SetParameter(0, mu_nom_data);
	kerw->SetParameter(1, sg_nom_data);
	...
	TH1D *h1d_kend_bb=new TH1D("h1d_kend_bb","", ny_edept, ymin_edept, ymax_edept);
	...
	for (Long64_t jentry=0; jentry<nentries;jentry++) { //evt loop
		...
		if (IsBeamMom&&IsBeamXY&&IsBQ&&IsCaloSize&&IsPandoraSlice) {
			...
			h1d_pbeam->Fill(1000.*p_beam, mom_rw_minchi2);

			h1d_kehy->Fill(fitted_KE, mom_rw_minchi2);
			h1d_phy->Fill(1000.*ke2p(fitted_KE/1000.), mom_rw_minchi2);
			...
			if (IsRecoStop) {
				...
				h1d_kebeam_stop->Fill(ke_beam_MeV, mom_rw_minchi2);
				h1d_pbeam_stop->Fill(1000.*p_beam, mom_rw_minchi2);

				h1d_phy_stop->Fill(1000.*ke2p(fitted_KE/1000.), mom_rw_minchi2);

				h1d_kehy_stop->Fill(fitted_KE, mom_rw_minchi2);

				h1d_keffbeam_stop->Fill(keffbeam, mom_rw_minchi2);

				h1d_dist_stop->Fill(d_sce);
				h1d_trklen_stop->Fill(range_reco, mom_rw_minchi2);
				h1d_kerange_stop->Fill(ke_range_MeV, mom_rw_minchi2);
				h1d_kecalo_stop->Fill(ke_calo_MeV, mom_rw_minchi2);
				...
				h1d_prange_stop->Fill(tmp_prange, mom_rw_minchi2);
				h1d_pcalo_stop->Fill(tmp_pcalo, mom_rw_minchi2);
		...
		//TFile *fout = new TFile(Form("/dune/data2/users/hyliao/protonana/v09_39_01/KEHY_KEBEAMFF_BKG/proton_beamxy_beammom_bkg_run%d.root",run),"RECREATE");
		TFile *fout = new TFile(Form("/dune/data2/users/hyliao/protonana/v09_39_01/KEFF_EDEPT/proton_beamxy_beammom_bkg_run%d.root",run),"RECREATE");
