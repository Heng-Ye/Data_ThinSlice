#define ProtonDemo_run5387_cxx
#include "ProtonDemo_run5387.h"
...
void ProtonDemo_run5387::Loop() {
	...
	std::cout<<"nentries: "<<nentries<<std::endl;
        ...
        TH2D *h2d_rr_dedx_recostop=new TH2D(Form("h2d_rr_dedx_recostop"), Form(""), n_b, b_min, b_max,n_dedx,dedx_min,dedx_max);
        //--------------------------------------------------------------------------------------------------------------------------//
	...
	for (Long64_t jentry=0; jentry<nentries;jentry++) { //evt loop
		...
		double yend_sce=0;
		double zend_sce=0;
		if (IsBQ&&IsCaloSize&&IsPandoraSlice) { //Basic cuts
			//calo info --------------------------------------------------------------------------//
	...
	//save results...
	h2d_rr_dedx_recostop->SetTitle("Stopping protons(Data); Residual Range[cm]; dE/dx [MeV/cm]");
	TFile *fout = new TFile(Form("data_proton_demo.root"),"RECREATE");
