...
void plot_BeamXY_Tail() {
	...
	//output folder
	TString fout="./plot_BeamXY/beamxy_data_mc.eps";
	...
	TH2D *bxy_mc=(TH2D*)f_mc->Get("bx_by_RecoEl");
	TH2D *bxy_mc_tail=(TH2D*)f_mc->Get("bx_by_RecoEl_tail");

	TH1D *keff_mc=(TH1D*)f_mc->Get("keffrange_RecoEl");
	TH1D *keff_mc_tail=(TH1D*)f_mc->Get("keffrange_RecoEl_tail");
	TH1D *keff_mc_bulk=(TH1D*)f_mc->Get("keffrange_RecoEl_bulk");
	
	//read data -----------------------------------------------------------------------//
	TFile *f_data = TFile::Open(fdata.Data());
	TH2D *bxy_data=(TH2D*)f_data->Get("bx_by_RecoEl");
	TH2D *bxy_data_tail=(TH2D*)f_data->Get("bx_by_RecoEl_tail");

	TH1D *keff_data=(TH1D*)f_data->Get("keffrange_RecoEl");
	TH1D *keff_data_tail=(TH1D*)f_data->Get("keffrange_RecoEl_tail");
	TH1D *keff_data_bulk=(TH1D*)f_data->Get("keffrange_RecoEl_bulk");
	...
	//TCanvas *c1=new TCanvas("c1","", 3600, 1200);
	...
	cout<<"double meanX_el_data="<<meanX_data<<endl;
	cout<<"double rmsX_el_data="<<rmsX_data<<endl;
	cout<<"double meanY_el_data="<<meanY_data<<endl;
	cout<<"double rmsY_el_data="<<rmsY_data<<endl;		


   	TEllipse *el_data = new TEllipse(meanX_data,meanY_data,1.5*rmsX_data,1.5*rmsY_data);
   	el_data->SetLineWidth(1);
	el_data->SetLineColor(2);
	...
	cout<<"double meanX_el_mc="<<meanX_mc<<endl;
	cout<<"double rmsX_el_mc="<<rmsX_mc<<endl;
	cout<<"double meanY_el_mc="<<meanY_mc<<endl;
	cout<<"double rmsY_el_mc="<<rmsY_mc<<endl;		



/*
	float meanY_data=bxy_data->GetYaxis()->GetMean();
	float rmsX_data=bxy_data->GetXaxis()->GetRMS();
	float rmsY_data=bxy_data->GetYaxis()->GetRMS();

   	TEllipse *el_data = new TEllipse(meanX_data,meanY_data,rmsX_data,rmsY_data);
   	el_data->SetLineWidth(1);
	el_data->SetLineolor(2);
	el_data->SetLineStyle(2);
*/

	c1->cd(1);
	TH2D *f2d_data=new TH2D("f2d_data","",n_bx,bx_min,bx_max,n_by,by_min,by_max);
	f2d_data->SetTitle("Reco. Elastic-scattering Protons [Data]; Beam X-position [cm]; Beam Y-position [cm]");
	//f2d_data->CenterTitle(true);
	f2d_data->Draw();
	...
	f2d_data2->SetTitle("Reco. Elastic-scattering Protons+KE_{ff}<280 MeV [Data]; Beam X-position [cm]; Beam Y-position [cm]");
	f2d_data2->Draw();
	bxy_data_tail->Draw("colz same");
	el_data->Draw("same");

	c1->cd(3);
	keff_data->SetLineColor(1);
	keff_data_tail->SetLineColor(2);
	keff_data_tail->SetFillColor(2);
	keff_data_bulk->SetLineColor(4);
	keff_data_bulk->SetFillStyle(3005);

	keff_data->SetLineWidth(1);
	keff_data_tail->SetLineWidth(1);
	keff_data_bulk->SetLineWidth(1);

	keff_data->SetTitle("TPC Front-Face Energy [Data]; Proton Kinetic Energy [MeV]; ");
	keff_data->GetXaxis()->SetRangeUser(kemin, ke_max);
	keff_data->Draw();
	keff_data_tail->Draw("same");
	keff_data_bulk->Draw("same");
	keff_data->Draw("same");

        TLegend *leg_data = new TLegend(0.14,0.65,.5,0.85);
	leg_data->SetFillStyle(0);
	leg_data->AddEntry(keff_data, "All reco. elastic", "l");
	leg_data->AddEntry(keff_data_tail, "Reco. elastic+KE_{FF}<280 MeV", "f");
	leg_data->AddEntry(keff_data_bulk, "Reco. elastic+Bulk Cut", "l");
	leg_data->Draw();
	...
	f2d_mc->SetTitle("Reco. Elastic-scattering Protons [MC]; Beam X-position [cm]; Beam Y-position [cm]");
	f2d_mc->Draw();
	...
	f2d_mc2->SetTitle("Reco. Elastic-scattering Protons+KE_{ff}<280 MeV [MC]; Beam X-position [cm]; Beam Y-position [cm]");
	f2d_mc2->Draw();
	bxy_mc_tail->Draw("colz same");
	el_mc->Draw("same");

	c1->cd(6);
	keff_mc->SetLineColor(1);
	keff_mc_tail->SetLineColor(2);
	keff_mc_tail->SetFillColor(2);
	keff_mc_bulk->SetLineColor(4);
	keff_mc_bulk->SetFillStyle(3005);
	keff_mc->SetTitle("TPC Front-Face Energy [MC]; Proton Kinetic Energy [MeV]; ");
	keff_mc->GetXaxis()->SetRangeUser(kemin, ke_max);

	keff_mc->SetLineWidth(1);
	keff_mc_tail->SetLineWidth(1);
	keff_mc_bulk->SetLineWidth(1);

	keff_mc->Draw("");
	keff_mc_tail->Draw("same");
	keff_mc_bulk->Draw("same");
	keff_mc->Draw("same");


        TLegend *leg_mc = new TLegend(0.14,0.65,.5,0.85);
	leg_mc->SetFillStyle(0);
	leg_mc->AddEntry(keff_mc, "All reco. elastic", "ep");
	leg_mc->AddEntry(keff_mc_tail, "Reco. elastic+KE_{FF}<280 MeV", "f");
	leg_mc->AddEntry(keff_mc_bulk, "Reco. elastic+Bulk Cut", "l");
	leg_mc->Draw();
