...
void plotSGBKGFitAfterBMRW(TString rep, TString rep1, TString x_axis_label, TString fdata, TString fmc, TString outpath) {
...
*/
	...
	for (int i = 1; i<=data->GetNbinsX(); ++i){
		...
		if (x_cen>xfit_min&&x_cen<xfit_max) { //ignore short track length area
			...
			double n_mc_other=n_mc_all-n_mc_el-n_mc_midp;
			double err_n_mc_other=sqrt(err_n_mc_all*err_n_mc_all+err_n_mc_el*err_n_mc_el+err_n_mc_midp*err_n_mc_midp);
			...
			h3->SetBinContent(i, n_mc_el);
			h3->SetBinError(i, err_n_mc_el);
	...
	//[2]Min.(h0-h1-a*h2-b*h3)
        ...
        fitter.SetHistograms(h0_data, h1_mcother, h2, h3); //h2:misidp ; h3:el
	...
	cout<<"scal.size():"<<scal.size()<<endl;
        ...
        cout<<"scal_el:"<<scal.at(1)<<" +- "<<err_scal.at(1)<<endl;
