...
void plotMisIDPBkgFitAfterBMRW(TString rep, TString rep1, TString x_axis_label, TString fdata, TString fmc, TString outpath) {
...
*/
	...
	for (int i = 1; i<=data->GetNbinsX(); ++i){
		...
		if (x_cen>xfit_min&&x_cen<xfit_max) { //ignore short track length area
			...
			double n_mc_misidp=mc_rw_midp->GetBinContent(i);
			double err_n_mc_misidp=mc_rw_midp->GetBinError(i);

			double n_mc_all=mc_rw->GetBinContent(i);
			double err_n_mc_all=mc_rw->GetBinError(i);
			
			double n_mc_other=n_mc_all-n_mc_misidp;
			double err_n_mc_other=sqrt(err_n_mc_all*err_n_mc_all+err_n_mc_misidp*err_n_mc_misidp);

			h1_mcother->SetBinContent(i, n_mc_other);
			h1_mcother->SetBinError(i, err_n_mc_other);

			h2->SetBinContent(i, n_mc_misidp);
			h2->SetBinError(i, err_n_mc_misidp);
	...
	double bestfit_misidp=av_R_rw;
	double err_bestfit_misidp=err_av_R_rw;
	cout<<"\n\nbestfit_misidp:"<<bestfit_misidp<<" +- "<<err_bestfit_misidp<<endl;

	TLine* l_misidp=new TLine(xfit_min, bestfit_misidp, xfit_max, bestfit_misidp);
	TLine* lup_misidp=new TLine(xfit_min, bestfit_misidp+n_sigma*err_bestfit_misidp, xfit_max, bestfit_misidp+n_sigma*err_bestfit_misidp);
	TLine* ldn_misidp=new TLine(xfit_min, bestfit_misidp-n_sigma*err_bestfit_misidp, xfit_max, bestfit_misidp-n_sigma*err_bestfit_misidp);
	...
	TLegendEntry* text_bestfit = leg2->AddEntry(l_misidp, Form("<Data/MC>: %.2f#pm%.2f",bestfit_misidp,err_bestfit_misidp),"l");
