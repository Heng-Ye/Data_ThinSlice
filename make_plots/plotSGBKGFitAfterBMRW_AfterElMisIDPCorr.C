...
void plotSGBKGFitAfterBMRW_AfterElMisIDPCorr(TString rep, TString rep1, TString x_axis_label, TString fdata, TString fmc, TString outpath) {
	//Proton scaling factors --------//
	//Proton corrections using BKG-rich channel
        //double scal_MisIDP=0.680456;
	//double err_scal_MisIDP=0.188598;
	//double scal_El=1;
	//double err_scal_El=0;	
	//
        double scal_MisIDP=1.64815;
	double err_scal_MisIDP=0.498966;
	double scal_El=0.959623;
	double err_scal_El=0.312441;	
	//-------------------------------//
	...
	for (int i = 1; i<=mc_rw_midp_orig->GetNbinsX(); ++i){
		//misidp
		double n=mc_rw_midp_orig->GetBinContent(i);
		double err_n=mc_rw_midp_orig->GetBinError(i);

		double n_corr=scal_MisIDP*n;
		double err_n_corr=n_corr*sqrt(pow(err_n/n,2)+pow(err_scal_MisIDP/scal_MisIDP,2));

		mc_rw_midp->SetBinContent(i, n_corr);
		mc_rw_midp->SetBinError(i, err_n_corr);

		//el
		double n2=mc_rw_el_orig->GetBinContent(i);
		double err_n2=mc_rw_el_orig->GetBinError(i);

		double n2_corr=scal_El*n2;
		double err_n2_corr=n_corr*sqrt(pow(err_n2/n2,2)+pow(err_scal_El/scal_El,2));

		mc_rw_el->SetBinContent(i, n2_corr);
		mc_rw_el->SetBinError(i, err_n2_corr);
...
*/
	...
	for (int i = 1; i<=data->GetNbinsX(); ++i){
		...
		//if (x_cen>trklen_thr) { //ignore short track length area
...
/*
	...
	//[3]Min.(h0-h1-s*h2_corr)
        ...
        fitter_aftermisidp_corr.Fit();
       	...
       	vector<double> scal_el_corr;
        vector<double> err_scal_el_corr;
        scal_el_corr.push_back(fitter_aftermisidp_corr.GetPar());
        err_scal_el_corr.push_back(fitter_aftermisidp_corr.GetParError());

	cout<<"scal_el_corr.size():"<<scal_el_corr.size()<<endl;
        cout<<"scal_el_corr:"<<scal_el_corr.at(0)<<" +- "<<err_scal_el_corr.at(0)<<endl;
