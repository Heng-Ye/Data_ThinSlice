...
#include "../headers/TemplateSimuFitter.h"

void plotInelMisIDPFitAfterBMRW(TString rep, TString rep1, TString x_axis_label, TString fdata, TString fmc, TString outpath) {
...
*/
	...
	TH1D *h2=new TH1D("h2","", data->GetNbinsX(), data->GetXaxis()->GetXmin(), data->GetXaxis()->GetXmax()); //midp
	TH1D *h3=new TH1D("h3","", data->GetNbinsX(), data->GetXaxis()->GetXmin(), data->GetXaxis()->GetXmax()); //el
	...
	for (int i = 1; i<=data->GetNbinsX(); ++i){
		...
		if (x_cen>xfit_min&&x_cen<xfit_max) { //ignore short track length area
			...
			double n_mc_inel=mc_rw_inel->GetBinContent(i);
			double err_n_mc_inel=mc_rw_inel->GetBinError(i);

			double n_mc_midp=mc_rw_midp->GetBinContent(i);
			double err_n_mc_midp=mc_rw_midp->GetBinError(i);

			double n_mc_all=mc_rw->GetBinContent(i);
			double err_n_mc_all=mc_rw->GetBinError(i);
			
			double n_mc_other=n_mc_all-n_mc_inel-n_mc_midp;
			double err_n_mc_other=sqrt(err_n_mc_all*err_n_mc_all+err_n_mc_inel*err_n_mc_inel+err_n_mc_midp*err_n_mc_midp);

			h1_mcother->SetBinContent(i, n_mc_other);
			h1_mcother->SetBinError(i, err_n_mc_other);

			h2->SetBinContent(i, n_mc_midp);
			h2->SetBinError(i, err_n_mc_midp);

			h3->SetBinContent(i, n_mc_inel);
			h3->SetBinError(i, err_n_mc_inel);
	...
	double bestfit_inel=av_R_rw;
	double err_bestfit_inel=err_av_R_rw;
	cout<<"\n\nbestfit_inel:"<<bestfit_inel<<" +- "<<err_bestfit_inel<<endl;

	TLine* l_inel=new TLine(xfit_min, bestfit_inel, xfit_max, bestfit_inel);
	TLine* lup_inel=new TLine(xfit_min, bestfit_inel+n_sigma*err_bestfit_inel, xfit_max, bestfit_inel+n_sigma*err_bestfit_inel);
	TLine* ldn_inel=new TLine(xfit_min, bestfit_inel-n_sigma*err_bestfit_inel, xfit_max, bestfit_inel-n_sigma*err_bestfit_inel);
	l_inel->SetLineColor(2);
	l_inel->SetLineWidth(3);
	lup_inel->SetLineColor(2); lup_inel->SetLineStyle(2);
	ldn_inel->SetLineColor(2); ldn_inel->SetLineStyle(2);
	c0->cd(2);
	l_inel->Draw("same");
	lup_inel->Draw("same");
	ldn_inel->Draw("same");
	...
	TLegendEntry* text_bestfit = leg2->AddEntry(l_inel, Form("<Data/MC>: %.2f#pm%.2f",bestfit_inel,err_bestfit_inel),"l");
	text_bestfit->SetTextColor(2);
        //leg2->AddEntry(l_inel, Form("Best-fit: %.2f#pm%.2f",scal_midp.at(0),err_scal_midp.at(0))," ");
        leg2->Draw();

	c0->Print(Form("%s",outpath.Data()));


	//[2]Min.(h0-h1-a*h2-b*h3)
        TemplateSimuFitter fitter;
        fitter.SetHistograms(h0_data, h1_mcother, h2, h3); //h2:misidp ; h3:inel
        //fitter.SetFitRange(4,-1);
        fitter.Fit();


       	vector<double> scal;
        vector<double> err_scal;
        scal=fitter.GetPar();
        err_scal=fitter.GetParError();

	cout<<"scal.size():"<<scal.size()<<endl;
        cout<<"scal_misidp:"<<scal.at(0)<<" +- "<<err_scal.at(0)<<endl;
        cout<<"scal_inel:"<<scal.at(1)<<" +- "<<err_scal.at(1)<<endl;
