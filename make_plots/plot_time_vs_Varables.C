...
void plot_time_vs_Varables() {
	...
	TH2D **h2d_time_pcaloOverprange_stop=new TH2D*[n];
	TH2D **h2d_time_pcalo_stop=new TH2D*[n];
	TH2D **h2d_time_prange_stop=new TH2D*[n];

	TH1D **h1d_zst_stop=new TH1D*[n];
	TF1 **fit_zst_stop=new TF1*[n];
	TText **txt_zst_stop=new TText*[n];
	vector<double> mu_zst_stop;
	vector<double> sigma_zst_stop;

	TH1D **h1d_zst_noSCE_stop=new TH1D*[n];
	TF1 **fit_zst_noSCE_stop=new TF1*[n];
	TText **txt_zst_noSCE_stop=new TText*[n];
	vector<double> mu_zst_noSCE_stop;
	vector<double> sigma_zst_noSCE_stop;
	...
	for (int i=0; i<n; ++i) {
		...
		h2d_time_pcaloOverprange_stop[i]=(TH2D *)file[i]->Get(Form("h2d_time_pcaloOverprange_stop"));
		h2d_time_pcalo_stop[i]=(TH2D *)file[i]->Get(Form("h2d_time_pcalo_stop"));
		h2d_time_prange_stop[i]=(TH2D *)file[i]->Get(Form("h2d_time_prange_stop"));
		...
		h1d_zst_stop[i]=(TH1D*)file[i]->Get(Form("h1d_zst_stop"));
		h1d_zst_noSCE_stop[i]=(TH1D*)file[i]->Get(Form("h1d_zst_noSCE_stop"));

		float ymax=h1d_zst_stop[i]->GetBinContent(h1d_zst_stop[i]->GetMaximumBin());
		float ymax_noSCE=h1d_zst_noSCE_stop[i]->GetBinContent(h1d_zst_noSCE_stop[i]->GetMaximumBin());

		float plot_ymax=ymax_noSCE;
		if (ymax>plot_ymax) plot_ymax=ymax;

		TH2D *f2d_zst_stop=new TH2D("f2d_zst_stop","",30,-10,40,plot_ymax,0,plot_ymax*1.1);
		f2d_zst_stop->SetTitle(Form("Run %d: t_{0}:%s (%.2f hrs); Z [cm]; Counts",RUN.at(i),buf_time, dt));


		TCanvas *c_zst=new TCanvas("c_zst","", 1200, 900);
		c_zst->Divide(1,1);
		c_zst->cd(1);
		TString str_zst_out;
		if (i==0) str_zst_out=Form("./zst_all.pdf(");
		if (i>0&&i<n-1) str_zst_out=Form("./zst_all.pdf");
		if(i==n-1) str_zst_out=Form("./zst_all.pdf)");
		f2d_zst_stop->Draw();
		h1d_zst_noSCE_stop[i]->SetLineColor(4);
		h1d_zst_stop[i]->Draw("same");
		h1d_zst_noSCE_stop[i]->Draw("same");

		fit_zst_stop[i]=VFit(h1d_zst_stop[i], 2);
		fit_zst_stop[i]->SetLineColor(2);
		fit_zst_stop[i]->SetLineStyle(2);
		fit_zst_stop[i]->Draw("same");

		mu_zst_stop.push_back(fit_zst_stop[i]->GetParameter(0));
		sigma_zst_stop.push_back(fit_zst_stop[i]->GetParameter(1));

		fit_zst_noSCE_stop[i]=VFit(h1d_zst_noSCE_stop[i], 3);
		fit_zst_noSCE_stop[i]->SetLineColor(3);
